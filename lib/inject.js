...
module.exports = function injectBinary(options, reqSelectors, resSelectors, htmlOnly) {
  ...
  function prepareResponseSelectors(req, res) {
    ...
    res.writeHead = function () {
      ...
      // These are the circumstances under which we will inject the zt-sdk-js
      if ( res.isHtml() && !res.isRedirect() && !res.isGzipped()) {
        ...
        var ztConfig = common.generateZitiConfigObject( '', req, options);
        ztConfig = JSON.stringify(ztConfig);
        ztConfig = Base64.encode(ztConfig);
        let domain;
        if (env('ZITI_BROWZER_BOOTSTRAPPER_WILDCARD_VHOSTS')) {
          domain = `${req.zt_vhost}.${ common.trimFirstSection( env('ZITI_BROWZER_BOOTSTRAPPER_HOST') )}`
        } else {
          domain = `${req.zt_vhost}`;
        }      
        cookies.push(
          cookie.serialize(
            '__zt-browzer-config', ztConfig, 
            {
              sameSite: true,
              path:     '/',
              domain:   `${domain}`,
    ...
    res.write = function (data, encoding) {
      ...
      // inject the zt-sdk-js until all redirects have completed, and we have a 200.
