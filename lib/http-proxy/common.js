...
var common   = exports,
    ...
    swpjson  = require('@hanzozt/zt-browzer-sw/package.json'),
...
var ztContextMutex = withTimeout(new Mutex(), 3 * 1000, new Error('timeout on ztContextMutex'));
...
common.generateZitiConfig = function(url, client) {
  ...
  var zt_config = `var ztConfig = ` + JSON.stringify(zc);
...
common.getZBRname = function() {
  ...
  try {
    ...
    fs.readdirSync(pathToZitiBrowzerRuntimeModule).forEach(file => {
      if (file.startsWith('zt-browzer-runtime')) {
        zbrName = file;
      }
    });
...
common.getZBRCSSname = function() {
  ...
  try {
    ...
    let zbrCSSName;
    fs.readdirSync(pathToZitiBrowzerRuntimeModule).forEach(file => {
      if (file.startsWith('zt-browzer-css-')) {
        zbrCSSName = file;
      }
    });
...
common.generateZitiConfigObject = function(url, req, options) {
  ...
  var idp_issuer_url = req.zt_idp_issuer_base_url;
  ...
  let selfHost;
  if (env('ZITI_BROWZER_BOOTSTRAPPER_WILDCARD_VHOSTS')) {
    selfHost = `${req.zt_vhost}.${ common.trimFirstSection( env('ZITI_BROWZER_BOOTSTRAPPER_HOST') )}`
  } else {
  ...
  var zt_config = 
    {
      controller: {
        api: `https://${zt_controller_host}:${zt_controller_port}/edge/client/v1`
      },
      browzer: {
        bootstrapper: {
          self: {
        ...
        sw: {
          location: `zt-browzer-sw.js`,
          version: `${swpjson.version}`, 
          logLevel: `${common.logLevelGetForClient(ztClient)}`,
        },
        runtime: {
      ...
      idp: {
        host: `${idp_issuer_url}`,
        clientId: `${req.zt_idp_client_id}`,
        authorization_endpoint_parms: req.zt_idp_authorization_endpoint_parms ? `${req.zt_idp_authorization_endpoint_parms}` : undefined,
        authorization_scope: req.zt_idp_authorization_scope ? `${req.zt_idp_authorization_scope}` : undefined,
      }
    };
...
common.isRequestOnTargetPath = function( req, options, path ) {
  ...
  if (req.zt_target_path) {
    let pathNoQuery = path.replace(/\?.*$/,"");
    let regex = new RegExp( pathNoQuery + '$', 'g' );
    let hit = (req.zt_target_path.match(regex) || []).length;
    if ((hit > 0)) {
...
common.addServerHeader = function(headerObj) {
  return Object.assign(headerObj, {
    'Server': `zt-browzer-bootstrapper/${pjson.version}`
  });
}


/** --------------------------------------------------------------------------------------------------
 *  Spin up a fresh ztContext 
 */
var ztContext;
common.newZitiContext = async ( logger ) => {

  await ztContextMutex.runExclusive(async () => {

    // If we have an active context, release any associated resources
    if (!isUndefined(ztContext)) {
        logger.silly({message: `now destroying ztContext[${ztContext._uuid}] - apiSessionHeartbeat has been cleared`});
        clearTimeout(ztContext.apiSessionHeartbeatId);
        ztContext.deleted = true;
        ztContext = undefined;
    }

    // Instantiate/initialize the ztContext we will use to obtain Service info from the Controller
    ztContext = new ZitiContext(Object.assign({
        logger:         logger,
    ...
    ztContext.on(ZITI_CONSTANTS.ZITI_EVENT_IDP_AUTH_HEALTH, common.idpAuthHealthEventHandler);

    await ztContext.ensureAPISession();
...
common.idpAuthHealthEventHandler = async ( idpAuthHealthEvent ) => {
    ...
    if (!idpAuthHealthEvent.ztContext.deleted) {
        if (idpAuthHealthEvent.expired) {
            common.newZitiContext( idpAuthHealthEvent.ztContext.logger );
        }
    }
...
common.getZitiContext = async function() {
  let ctx;
  await ztContextMutex.runExclusive( () => {
    ctx = ztContext;
  });
  return ctx;
}
