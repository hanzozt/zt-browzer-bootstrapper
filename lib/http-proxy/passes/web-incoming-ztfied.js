...
module.exports = {
  ...
  stream: async function stream(req, res, options, _, server, clb) {
    ...
    if (env('ZITI_BROWZER_BOOTSTRAPPER_WILDCARD_VHOSTS') && env('NOT_YET_IMPLEMENTED')) {
      ...
      let ztContext = await common.getZitiContext();
      let id = await ztContext.getServiceIdByName( req.zt_target_service );
      ...
      if (isUndefined(id)) {  // if not...
        ...
        do {
          ztContext = await common.getZitiContext();
          fetchComplete = await ztContext.fetchServiceByName( req.zt_target_service );
          if (!fetchComplete) {
        ...
        ztContext = await common.getZitiContext();
        id = await ztContext.getServiceIdByName( req.zt_target_service );
        ...
        if (isUndefined(id)) {  // if not, route the HTTP request to a 404 response
          ...
          let browzer_error_data_json = {
            ...
            myvar: {
              type: 'zrok',
              zrokshare: `${req.zt_target_service}`,
            }
          };    

          res.writeHead( 301,
            { 
              'X-Ziti-BrowZer-Bootstrapper': `target zrok share [${req.zt_target_service}] unknown`,
              'Location': `/browzer_error?browzer_error_data=${stringify(JSON.stringify(browzer_error_data_json))}` 
            }
          );
          res.end('');
          if (!net.isIP(req.headers['host'])) {
            options.logger.warn({message: `req terminate - target zrok share [${req.zt_target_service}] unknown`, vhost: req.headers['host']});
          }
          return;
    ...
    //
    if (typeof req.zt_target_service === 'undefined') {
    ...
    //
    options.target = 'https://' + req.zt_target_service;
    options.targetPath = req.zt_target_path;
    ...
    //
    let rtNakedRequest = (outgoing.path.match(/\/zt-browzer-runtime\.js/) || []).length;
    if ((rtNakedRequest > 0)) {
      options.logger.silly({message: 'Request for naked zt-browzer-runtime.js', component: 'JS', clientIp: requestIp.getClientIp(req), method: req.method, url: req.url});

      // Locate the path to the distro within the build of our running instance
      let pathToZitiBrowzerRuntimeModule;
      pathToZitiBrowzerRuntimeModule = require.resolve('@hanzozt/zt-browzer-runtime');
      ...
      options.logger.silly({message: 'zt-browzer-runtime SHA filename: ' + rtFileName, clientIp: requestIp.getClientIp(req), method: req.method, url: req.url});

      res.writeHead(302, common.addServerHeader({
        'Location': `${rtFileName}`,
        'X-Ziti-BrowZer-Bootstrapper': 'zt-browzer-runtime SHA filename',
      }));
      res.end('');
    ...
    //
    let corsProxyRequest = (outgoing.path.match(/\/zt-cors-proxy\//) || []).length;
    ...
    //
    let rtRequest = (outgoing.path.match(/\/zt-browzer-runtime/) || []).length;
    if ((rtRequest > 0)) {
      options.logger.silly({message: 'Request for zt-browzer-runtime', component: 'JS', clientIp: requestIp.getClientIp(req), method: req.method, url: req.url});
    ...
    //
    let cssRequest = (outgoing.path.match(/\/zt-browzer-css/) || []).length;
    if ((cssRequest > 0)) {
      options.logger.silly({message: 'Request for zt-browzer-css.css', component: 'CSS', clientIp: requestIp.getClientIp(req), method: req.method, url: req.url});

      // Locate the path to the distro within the build of our running instance
      let pathToZitiBrowzerRuntimeModule = require.resolve('@hanzozt/zt-browzer-runtime');
      pathToZitiBrowzerRuntimeModule = pathToZitiBrowzerRuntimeModule.substring(0, pathToZitiBrowzerRuntimeModule.lastIndexOf('/'));
    ...
    //
    let curRequest = (outgoing.path.match(/\/zt-browzer-cur/) || []).length;
    if ((curRequest > 0)) {
      options.logger.silly({message: 'Request for zt-browzer-cur.cur', component: 'CUR', clientIp: requestIp.getClientIp(req), method: req.method, url: req.url});
      ...
      // Read the component off the disk
      let curFileName = 'zt-browzer-cur.cur';
    ...
    //
    let logoRequest = (outgoing.path.match(/\/zt-browzer-logo.svg/) || []).length;
    if ((logoRequest > 0)) {
      options.logger.silly({message: 'Request for zt-browzer-logo.svg', component: 'SVG', clientIp: requestIp.getClientIp(req), method: req.method, url: req.url});
      ...
      // Read the component off the disk
      fs.readFile( path.join( pathToZitiBrowzerRuntimeModule, 'zt-browzer-logo.svg' ), (err, data) => {
    ...
    if ((rtwasmRequest > 0)) {
      options.logger.silly({message: 'Request for zt-browzer-runtime libcrypto.wasm', component: 'WASM', clientIp: requestIp.getClientIp(req), method: req.method, url: req.url});

      // Locate the path to the SW distro within the build of our running instance
      let pathToZitiBrowzerRuntimeModule = require.resolve('@hanzozt/libcrypto-js');
      pathToZitiBrowzerRuntimeModule = pathToZitiBrowzerRuntimeModule.substring(0, pathToZitiBrowzerRuntimeModule.lastIndexOf('/'));
        ...
        if (err) {  // If we can't read the file from disk
          ...
          var cacheData = {
            ...
            headers: common.addServerHeader({ 
              ...
              'X-Ziti-BrowZer-Bootstrapper': 'Hanzo ZT browZer WebAssembly',
              }),
            data: data
          }
    ...
    if ((polipopRequest > 0)) {
      options.logger.silly({message: 'Request for zt-browzer-UI', component: 'UI', clientIp: requestIp.getClientIp(req), method: req.method, url: req.url});
        ...
        if (err) {  // If we can't read the file from disk
          ...
          var cacheData = {
            ...
            headers: common.addServerHeader({ 
              ...
              'X-Ziti-BrowZer-Bootstrapper': 'Hanzo ZT browZer Polipop',
            }),
            data: data
          }
    ...
    //
    let swRequest = (outgoing.path.match(/\/zt-browzer-sw/) || []).length;
    if ((swRequest > 0)) {
      options.logger.silly({message: 'Request for zt-browzer-sw', component: 'JS', clientIp: requestIp.getClientIp(req), method: req.method, url: req.url});
        ...
        if (err) {  // If we can't read the file from disk
          ...
          var cacheData = {
            ...
            headers: common.addServerHeader({ 
              ...
              'X-Ziti-BrowZer-Bootstrapper': 'Hanzo ZT browZer Service Worker',
            }),
            data: data
          }
...
<html>
  <head>
    <title>Hanzo ZT BrowZer Bootstrapper</title>
  </head>
  <body></body>
</html>
`;
